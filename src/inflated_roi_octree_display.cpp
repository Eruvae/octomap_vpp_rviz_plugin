#include "octomap_vpp_rviz_plugin/inflated_roi_octree_display.h"
...
void InflatedRoiOctreeDisplay::incomingMessageCallback(const octomap_msgs::OctomapConstPtr& msg)
{
  ...
  // creating octree
  octomap_vpp::InflatedRoiOcTree* octomap = NULL;
  ...
  size_t pointCount = 0;
  {
    ...
    for (octomap_vpp::InflatedRoiOcTree::iterator it = octomap->begin(treeDepth), end = octomap->end(); it != end; ++it)
    {
          ...
          for (unsigned int idxCase = 0; idxCase < 3; ++idxCase)
          {
            ...
            for (int i = 0; allNeighborsFound && i < 2; ++i)
            {
              ...
              for (key[idx_1] = nKey[idx_1] + diff[0] + 1; allNeighborsFound && key[idx_1] < nKey[idx_1] + diff[1]; key[idx_1] += stepSize)
              {
                ...
                {
                  octomap_vpp::InflatedRoiOcTreeNode* node = octomap->search(key, treeDepth);
...
void InflatedRoiOctreeDisplay::setVoxelColor(rviz::PointCloud::Point& newPoint, octomap_vpp::InflatedRoiOcTreeNode& node, double minZ, double maxZ, float maxRoiVal)
{
