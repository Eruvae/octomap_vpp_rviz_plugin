#include "octomap_vpp_rviz_plugin/roi_octree_display.h"
...
void RoiOcTreeDisplay::incomingMessageCallback(const octomap_msgs::OctomapConstPtr& msg)
{
  ...
  // creating octree
  octomap_vpp::RoiOcTree* octomap = NULL;
  ...
  size_t pointCount = 0;
  {
    ...
    for (octomap_vpp::RoiOcTree::iterator it = octomap->begin(treeDepth), end = octomap->end(); it != end; ++it)
    {
            ...
            for (unsigned int idxCase = 0; idxCase < 3; ++idxCase)
            {
              ...
              for (int i = 0; allNeighborsFound && i < 2; ++i)
              {
                ...
                for (key[idx_1] = nKey[idx_1] + diff[0] + 1; allNeighborsFound && key[idx_1] < nKey[idx_1] + diff[1]; key[idx_1] += stepSize)
                {
                  ...
                  {
                    octomap_vpp::RoiOcTreeNode* node = octomap->search(key, treeDepth);
...
void RoiOcTreeDisplay::setVoxelColor(rviz::PointCloud::Point& newPoint, octomap_vpp::RoiOcTreeNode &node, double minZ, double maxZ)
{
