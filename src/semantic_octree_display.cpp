#include "octomap_vpp_rviz_plugin/semantic_octree_display.h"
...
SemanticOcTreeDisplay::SemanticOcTreeDisplay() : octomap_rviz_plugin::OccupancyGridDisplay()
{
  // TODO: only render selected class: octree_render_property_->addOption( "Class Voxels",  OCTOMAP_CLASS_VOXELS);
  ...
  octree_coloring_property_->addOption( "Classes",  OCTOMAP_CLASS_COLOR );

  min_color_property_ = new rviz::ColorProperty("Minimum color", Qt::red, "Color used for free voxels, depending on coloring method", this, SLOT(updateTopic));
  max_color_property_ = new rviz::ColorProperty("Maximum color", Qt::green, "Color used for occupied voxels, depending on coloring method", this, SLOT(updateTopic));
}

void SemanticOcTreeDisplay::incomingMessageCallback(const octomap_msgs::OctomapConstPtr& msg)
{
  ++messages_received_;
  ...
  // creating octree
  octomap_vpp::SemanticOcTree* octomap = NULL;
  octomap::AbstractOcTree* tree = octomap_msgs::msgToMap(*msg);
  if (tree){
    octomap = dynamic_cast<octomap_vpp::SemanticOcTree*>(tree);
    if(!octomap){
      setStatusStd(StatusProperty::Error, "Message", "Wrong octomap type. Use a different display type.");
      return;
    }
  ...
  size_t pointCount = 0;
  {
    ...
    for (octomap_vpp::SemanticOcTree::iterator it = octomap->begin(treeDepth), end = octomap->end(); it != end; ++it)
    {
        if(it.getZ() <= maxHeight && it.getZ() >= minHeight)
        {
          ...
          // evaluates to 1 for free voxels and 2 for occupied voxels
          int cur_node_mode_mask = ((int)octomap->isNodeOccupied(*it) + 1);
            ...
            for (unsigned int idxCase = 0; idxCase < 3; ++idxCase)
            {
              ...
              for (int i = 0; allNeighborsFound && i < 2; ++i)
              {
                ...
                for (key[idx_1] = nKey[idx_1] + diff[0] + 1; allNeighborsFound && key[idx_1] < nKey[idx_1] + diff[1]; key[idx_1] += stepSize)
                {
                  ...
                  {
                    octomap_vpp::SemanticOcTreeNode* node = octomap->search(key, treeDepth);
...
void SemanticOcTreeDisplay::setVoxelColor(rviz::PointCloud::Point& newPoint, octomap_vpp::SemanticOcTreeNode &node, double minZ, double maxZ)
{
...
bool SemanticOcTreeDisplay::checkType(std::string type_id)
{
  if(type_id == "SemanticOcTree") return true;
  else return false;
}
...
PLUGINLIB_EXPORT_CLASS( octomap_vpp_rviz_plugin::SemanticOcTreeDisplay, rviz::Display)
